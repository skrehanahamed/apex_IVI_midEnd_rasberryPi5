...
scp -o BatchMode=yes -o ConnectTimeout=20 "${SCRIPT_DIR}/10-audio-stability.conf" "root@${PI_IP}:/etc/pipewire/pipewire.conf.d/10-audio-stability.conf"
scp -o BatchMode=yes -o ConnectTimeout=20 "${SCRIPT_DIR}/20-ivi-hdmi.conf"      "root@${PI_IP}:/etc/pipewire/pipewire.conf.d/20-ivi-hdmi.conf"
scp -o BatchMode=yes -o ConnectTimeout=20 "${SCRIPT_DIR}/10-bluez-ofono.conf"   "root@${PI_IP}:/etc/wireplumber/wireplumber.conf.d/10-bluez-ofono.conf"
...
scp -o BatchMode=yes -o ConnectTimeout=20 "${DEPLOY_DIR}/ApexIVI"                "root@${PI_IP}:/tmp/ApexIVI"
scp -o BatchMode=yes -o ConnectTimeout=20 "${SCRIPT_DIR}/apex-fetch-artwork.py"  "root@${PI_IP}:/usr/bin/apex-fetch-artwork.py"
ssh "root@${PI_IP}" "chmod +x /usr/bin/apex-fetch-artwork.py"

echo "🔄 [4/4] Restarting apex-ivi service on Pi..."
ssh "root@${PI_IP}" "systemctl stop apex-ivi && mv /tmp/ApexIVI /usr/bin/ApexIVI && chmod +x /usr/bin/ApexIVI && systemctl start apex-ivi && systemctl is-active apex-ivi"
